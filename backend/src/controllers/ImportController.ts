...
export const validateAndImport = async (req: Request, res: Response): Promise<void> => {
  const { fileId, requiredFields, fieldTypes, collection } = req.body as ValidateAndImportRequest;
  ...
  if (!requiredFields || !Array.isArray(requiredFields) || requiredFields.length === 0) {
    res.status(400).json({ 
      status: 'error',
      message: 'requiredFields is required and must be a non-empty array' 
    });
    return;
  }
  ...
  if (!fieldTypes || typeof fieldTypes !== 'object') {
    res.status(400).json({ 
      status: 'error',
      message: 'fieldTypes is required and must be an object' 
    });
    return;
  }
  ...
  try {
    ...
    const { insertedCount, collectionStatus } = await insertRowsToDB(collection, validRows);
