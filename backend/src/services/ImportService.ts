...
export const insertRowsToDB = async (
  ...
  const collectionStatus = collectionNames.includes(sheetName) ? 'collection found' : 'collection created';

  const result = await db.collection(sheetName).insertMany(validRows);
