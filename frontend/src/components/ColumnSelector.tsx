  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (error && typeof error === 'object' && 'isValidationError' in error) {
        setImportResult(error as unknown as ImportResult); // Set the validation error result to display to user
        const errorObj = error as { message?: string };
        toast.error(`Import failed: ${errorObj.message || 'Data validation errors found'}`);
      } else {
